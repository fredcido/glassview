...
dhtmlXGridObject.prototype.submitOnlyCheckedId = function(mode)
{
	this.FormSubmitOnlyCheckedId = convertStringToBoolean(mode);
        if ( this.FormSubmitOnlyCheckedId ) {
            var self = this;
            dhtmlxEvent(this, 'check',function() {if (self.entBox) self.parentFormOnSubmit();});
            dhtmlxEvent(this, 'clearAll',function() { this.clearFields(); });
        }
}

dhtmlXGridObject.prototype.clearFields = function()
{
    if ( this.parentForm == false ) {
            return false;
    }
...
dhtmlXGridObject.prototype.createFormInput = function(name,value){
    ...
    if (this._input_mask)
    	...
    	input.name =((this.globalBox||this.entBox).id||'dhtmlXGrid')+'[]';
    input.value = value;
...
dhtmlXGridObject.prototype.formCreateInputCollection = function()
{
    ...
    } else if ( this.FormSubmitOnlyCheckedId ) {
        var checkedRows = this.getCheckedRows(0);
        for(var i=0;i<checkedRows.length;i++)
            this.createFormInput("selected",checkedRows[i]);
