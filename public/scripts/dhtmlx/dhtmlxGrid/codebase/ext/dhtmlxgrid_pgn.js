...
dhtmlXGridObject.prototype.enablePaging = function(fl,pageSize,pagesInGrp,parentObj,showRecInfo,recInfoParentObj){
    ...
    if (arguments.length==0)pageNum=this.currentPage||0;pageNum=parseInt(pageNum);pageNum=Math.max(1,Math.min(pageNum,Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize)));if(!this.callEvent("onBeforePageChanged",[this.currentPage,pageNum]))
        return;this.currentPage = parseInt(pageNum);this._reset_view();this.callEvent("onPageChanged",this.getStateOfView())
    ...
    if (!this.pagesInGroup)this.pagesInGroup=Math.ceil(Math.min(5,this.rowsBuffer.length/this.rowsBufferOutSize));var totalPages=Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize);if (totalPages && totalPages<this.currentPage)return this.changePage(totalPages);if (this.pagingOn && this._pgn_skin)this._pgn_skin.apply(this,this.getStateOfView())
    ...
    if (!this.pagingBlock){
        this.pagingBlock = document.createElement("DIV");this.pagingBlock.className = "pagingBlock";this.recordInfoBlock = document.createElement("SPAN");this.recordInfoBlock.className = "recordsInfoBlock";if (!this._pgn_parentObj)return;this._pgn_parentObj.appendChild(this.pagingBlock)
        if(this._pgn_recInfoParentObj)this._pgn_recInfoParentObj.appendChild(this.recordInfoBlock)


        if (!this._pgn_templateA){
            imgnext = "<img src=\"" + this.imgURL + "ar_right_dis.gif\" align=\"absbottom\" />";
            imgprev = "<img src=\"" + this.imgURL + "ar_left_dis.gif\" align=\"absbottom\" />";

            this._pgn_templateA=this._pgn_template_compile("[prevpages:" + imgprev + ":&nbsp;] [currentpages:,&nbsp;] [nextpages:" + imgnext + ":&nbsp;]");this._pgn_templateB=this._pgn_template_compile("Resultados <b>[from]-[to]</b> de <b>[total]</b>")
        }
    ...
    var start=Math.floor((this.currentPage-1)/this.pagesInGroup)*this.pagesInGroup;var max=Math.min(Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize),start+this.pagesInGroup);var str=[];for (var i=start+1;i<=max;i++)str.push("<a href='javascript:void(0);' onclick='this.grid.changePage("+i+")'>"+(i==this.currentPage?("<b>"+i+"</b>"):i)+"</a>");return str.join(sep)
    };dhtmlXGridObject.prototype._pgn_link=function(mode,ac,ds){
    if (mode=="prevpages" || mode=="prev"){
        if (this.currentPage==1)return ds;return '<a href=\'javascript:void(0);\' onclick=\'this.grid.changePageRelative(-1*this.grid.pagesInGroup)\'>'+ac+'</a>'
    };if (mode=="nextpages" || mode=="next"){
        if (this.rowsBuffer.length/this.rowsBufferOutSize <= this.currentPage )return ds;return '<a href=\'javascript:void(0);\' onclick=\'this.grid.changePageRelative(this.grid.pagesInGroup)\'>'+ac+'</a>'
    };if (mode=="current"){
        var i=this.currentPage+(ac?parseInt(ac):0);if (i<1 || Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize)< i ) return ds;return '<a href=\'javascript:void(0);\' '+(i==this.currentPage?"class='dhx_active_page_link' ":"")+'onclick=\'this.grid.changePage('+i+')\'>'+i+'</a>'
    };return ac
};dhtmlXGridObject.prototype._pgn_template_active=function(block){
    var tags=block.getElementsByTagName("A");if (tags)for (var i=0;i < tags.length;i++){
        tags[i].grid=this
    }
    ...
    if (!this.aToolBar)this.aToolBar=this._pgn_createAToolBar();var totalPages=Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize);if (this._WTDef[2]){
        this.aToolBar.enableItem("right");this.aToolBar.enableItem("rightabs");this.aToolBar.enableItem("left");this.aToolBar.enableItem("leftabs");if(this.currentPage==totalPages){
            this.aToolBar.disableItem("right");this.aToolBar.disableItem("rightabs")
        }else{
            if(this.currentPage==1){
                this.aToolBar.disableItem("left");this.aToolBar.disableItem("leftabs")
            }
        };var pButton = this.aToolBar.getItem("pages")
        pButton.clearOptions();for(var i=0;i<totalPages;i++){
            pButton.addOption((i+1),this.i18n.paging.page+(i+1))
        };pButton.setSelected(page.toString())
    };if (this._WTDef[1]){
        var iButton = this.aToolBar.getItem("results");if (!this.getRowsNum())
            iButton.setText(this.i18n.paging.notfound);else
            iButton.setText(this.i18n.paging.records+(start+1)+this.i18n.paging.to+end)
    };if (this._WTDef[3])this.aToolBar.getItem("perpagenum").setSelected(this.rowsBufferOutSize.toString())
...
};dhtmlXGridObject.prototype._pgn_createAToolBar = function(){
    if (!this._WTDef)this.setPagingWTMode(true,true,true,true);this.aToolBar=new dhtmlXToolbarObject(this._pgn_parentObj,'100%',22,"Grid Output");var self=this;var f1=function(val){
        switch (this.id){
            case "leftabs":
                ...
                self.changePage(val);break
        }
    };this.aToolBar.showBar();if (this._WTDef[0]){
        this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL+'ar_left_abs.gif',"18px","18px",f1,'leftabs',this.i18n.paging.first))
    ...
    if (this._WTDef[0]){
        this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL+'ar_right.gif',"18px","18px",f1,'right',this.i18n.paging.next))
    ...
    if(tmpAr.length>1 && (tmp=="light" || tmp=="modern"))
        ...
        var lineWidth = 20;var lineWidthInc = 22;if(page>this.pagesInGroup){
            ...
            var pageCont = document.createElement("DIV");var pageBox = document.createElement("DIV");pageCont.className = "dhx_page"+sfx;pageNumber = ((Math.ceil(page/this.pagesInGroup)-1)*this.pagesInGroup)+i;if(pageNumber>Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize))
                break;pageBox.innerHTML = pageNumber;pageCont.appendChild(pageBox);if(page==pageNumber){
                pageCont.className += " dhx_page_active"+sfx;pageBox.className = "dhx_page_active"+sfx
            }else{
                var self = this;pageCont.pgnum = pageNumber;pageCont.onclick = function(){
                    self.changePage(this.pgnum)
                }
            ...
            var pageCont = document.createElement("DIV");var pageBox = document.createElement("DIV");pageCont.className = "dhx_page"+sfx;pageBox.innerHTML = "&rarr;";pageCont.appendChild(pageBox);this.pagerElAr["pagerBox"].appendChild(pageCont);var self = this;pageCont.pgnum = (Math.ceil(page/this.pagesInGroup)*this.pagesInGroup)+1;pageCont.onclick = function(){
                self.changePage(this.pgnum)
            };lineWidth +=lineWidthInc
        };this.pagerElAr["pagerLine"].style.width = lineWidth+"px"
    };if(this.rowsBuffer.length>0 && this.showRecInfo==true)this.pagerElAr["pagerInfoBox"].innerHTML = this.i18n.paging.records+(start+1)+this.i18n.paging.to+end+this.i18n.paging.of+this.rowsBuffer.length;else if(this.rowsBuffer.length==0){
        this.pagerElAr["pagerLine"].parentNode.removeChild(this.pagerElAr["pagerLine"]);this.pagerElAr["pagerInfoBox"].innerHTML = this.i18n.paging.notfound
    };this.pagerElAr["pagerBox"].appendChild(document.createElement("SPAN")).innerHTML = "&nbsp;";this.pagerElAr["pagerBord"].appendChild(document.createElement("SPAN")).innerHTML = "&nbsp;";this.pagerElAr["pagerCont"].appendChild(document.createElement("SPAN")).innerHTML = "&nbsp;"
};
//v.1.6 build 80512
