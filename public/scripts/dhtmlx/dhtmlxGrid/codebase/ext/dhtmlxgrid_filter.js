...
dhtmlXGridObject.prototype.filterBy=function(column, value, preserve){if (this.isTreeGrid()) return this.filterTreeBy(column, value, preserve);if (this._f_rowsBuffer){if (!preserve){this.rowsBuffer=dhtmlxArray([].concat(this._f_rowsBuffer));if (this._fake)this._fake.rowsBuffer=this.rowsBuffer;}}else
 ...
 this.attachEvent("onCellChanged",f),
...
dhtmlXGridObject.prototype.formatCurrency = function(num, decimal, milhar, mask)  {
    decimal = decimal == undefined ? ',' : decimal;
    milhar = milhar == undefined ? '.' : milhar;
    ...
    for (var i = 0; i < Math.floor((num.length-(1+i))/3); i++)
	    num = num.substring(0,num.length-(4*i+3)) + milhar + num.substring(num.length-(4*i+3));

    var val = ( ( (sign)?'':'-') + ( mask ? 'R$' : '' ) + num + decimal + cents);
    return val;
};
dhtmlXGridObject.prototype.unformatCurrency = function( num )
{
    return num == '-' ? 0 : objGeral.toFloat(num);   
}
//v.2.6 build 100722
