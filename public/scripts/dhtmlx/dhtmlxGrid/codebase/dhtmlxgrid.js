...
var globalActiveDHTMLGridObject;String.prototype._dhx_trim=function(){return this.replace(/&nbsp;/g, " ").replace(/(^[ \t]*)|([ \t]*$)/g, "")};function dhtmlxArray(ar){return dhtmlXHeir((ar||new Array()), dhtmlxArray._master)};dhtmlxArray._master={_dhx_find:function(pattern){for (var i = 0;i < this.length;i++){if (pattern == this[i])return i};return -1},
 ...
 null
  ...
  load:function(url, call, type){this.callEvent("onXLS", [this]); this.argumentsLoad = arguments; if (arguments.length == 2 && typeof call != "function"){type=call;};type=type||"xml";if (!this.xmlFileUrl)this.xmlFileUrl=url;this._data_type=type;this.xmlLoader.onloadAction=function(that, b, c, d, xml){if (!that.callEvent)return;xml=that["_process_"+type](xml);if (!that._contextCallTimer)that.callEvent("onXLE", [that,0,0,xml]);if (call){call();}};this.xmlLoader.loadXML(url);},

 reload: function( flag )
 {
     if ( this.argumentsLoad ) {

         if ( objGeral.empty( flag ) )
             objGeral.loading( true );
	 
         this.clearAll();
         this.load.apply( this, this.argumentsLoad );
     }
 },
